# Script para generar los manuales PDF de badea desde a_badea.json con QR y branding MELANT IA
...
def main():
    with open("a_badea.json", encoding="utf-8") as f:
        data = json.load(f)
    # Manual regenerativo
    mr = data["manual_regenerativo"]
    generar_pdf(mr["titulo"], mr["contenido"], mr["qr_link"], "manual_badea_regenerativo.pdf")
    # Manual químico
    mq = data["manual_quimico"]
    generar_pdf(mq["titulo"], mq["contenido"], mq["qr_link"], "manual_badea_quimico.pdf")
